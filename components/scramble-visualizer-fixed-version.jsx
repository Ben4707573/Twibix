...
import { SVG, drawNNNCube } from './scramblers/cstimer/draw-utils';
...
export function CSTimerCubeVisualizer({ scramble, cubeType }) {
  ...
  const getInitialCubeState = () => {
    ...
    // Each face is represented as an array of stickers
    const state = {};
    
    // Validate size
    const cubeSize = Math.max(2, Math.min(7, size)); // Ensure size is between 2 and 7
    
    // Initialize each face with its own color
    for (const face of 'URFDLB') {
      state[face] = Array(cubeSize * cubeSize).fill(face);
    }
  ...
  const applyMoves = (state, moves) => {
    for (let move of moves) {
      ...
      if (!face || !state[face]) {
        console.warn(`Skipping invalid or unknown move: '${move}' (parsed face: '${face}')`);
      ...
      try {
        ...
        console.error(`Error applying move '${move}' to face '${face}':`, err);
      }
    }
  };
  ...
  const rotateFace = (state, face) => {
    try {
      // Safety check: ensure the face exists and is an array
      if (!state[face] || !Array.isArray(state[face])) {
        console.error(`Invalid face: ${face} is not an array`, state[face]);
        return;
      }
      ...
      console.error(`Error rotating face '${face}':`, err);
    }
  };
